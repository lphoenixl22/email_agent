...
class LLMAnalyzer:
    ...
    def _get_system_prompt(self) -> str:
        ...
        return """Ты - преподаватель, который проверяет студенческие лабораторные работы по MikroTik RouterOS.
Твоя задача - объективно оценить отчет студента по заданным критериям.

Особенности проверки работ по MikroTik:
1. Проверяй наличие и корректность команд RouterOS
2. Обращай внимание на использование safe mode (<SAFE>)
3. Проверяй правильность настройки IP адресов и маршрутов
4. Для старших лабораторных (>=2) обязательно проверяй статические маршруты
5. Анализируй скриншоты (если есть) - топология, пинги, интерфейс WinBox
6. Следи за плагиатом - если конфигурации студентов идентичны, это подозрительно
...
"""
    ...
    def _build_prompt(self, report_text: str, lab_requirements: Dict, 
        ...
        # Проверка на плагиат
        plagiarism_warning = ""
        if student_info.get('plagiarism_warning'):
            plagiarism_warning = f"\n\n{student_info['plagiarism_warning']}\n" \
                                f"Учти это при выставлении оценки - снизь балл за плагиат!\n"
